const salesInvoice = Vue.component('sales-invoice', {
    template: `
        <div>
            <div class="row">
                <div class="col-xs-12">
                    <form v-on:submit.prevent="downloadInvoicePdf">
                        <a href="" v-on:click.prevent="print"><i class="fa fa-print"></i> Print</a>
                        &nbsp;&nbsp;<input type="email" v-model="sales.Customer_Email" style="display: none;">
                        <button type="submit"><i class="fa fa-download"></i> PDF</button>
                    </form?
                </div>
            </div>
                    ...
                    <div class="col-xs-6">
                        <table _t92sadbc2>
                            ...
                            <tr>
                                ...
                                <td style="text-align:right">{{ sales.labourCost }}</td>
                            </tr>
    ...
    methods:{
        ...
        deletePdf(path) {
            axios.post('/delete_invoice_pdf', {path: path})
            .then(res => {
                let r = res.data;
                console.log(r);
            })
        ...
        downloadInvoicePdf() {
            let data = {
                invoice: this.sales.SaleMaster_SlNo, 
                InWord:this.convertNumberToWords(this.sales.SaleMaster_TotalSaleAmount)
            }

            axios.post('/download_invoice', data).then(res => {
                let r = res.data;
                let url = location.origin;
                if(r.success) {
                    var link = document.createElement('a');
                    link.href = url+r.filePath;
                    link.download = r.name;
                    link.dispatchEvent(new MouseEvent('click'));
                    this.deletePdf(r.filePath);
                    alert(r.message);
                }
            })
        },
