...
//    }
    ...
    public function invoiceDownload($name) 
    {
    ...
    public function downloadInvoicePdf()
    {
        ...
        try {
            ...
            select 
                sd.*,
                concat(p.Product_Name, ' - ', p.Product_Code) as details
            from tbl_saledetails sd 
            left join tbl_product p on p.Product_SlNo = sd.Product_IDNo
            where sd.SaleMaster_IDNo = ?
            ", $data->invoice);

            $sale['sales'] = $sales->row();
            $sale['saleDetails'] = $saleDetails->result();
            $sale['in_word'] = $data->InWord;
            $this->load->library('pdf');
            $view = $this->load->view('Administrator/sales/pdf_invoice', $sale, true);
            $this->pdf->createPDF($view, 'Mitfort-Invoice-'.$data->invoice, false);
 
            $filePath = '/uploads/Mitfort-Invoice-'.$data->invoice.'.pdf';
            $name = 'Mitfort-Invoice-'.$data->invoice.'.pdf';
         
            $res = ['success'=>true, 'message'=>'Invoice PDF Download!', 'filePath' => $filePath, 'name' => $name];
    ...
    public function deleteInvoicePdf() 
    {
        ...
        try {
            ...
            if($filePath){
                unlink($filePath);
                $res = ['success'=>true, 'message'=>'Invoice PDF Delete!'];
            } else {
                $res = ['success'=>true, 'message'=>'Invoice PDF Delete Failed!'];
            }
