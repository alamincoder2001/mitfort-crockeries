						...
						<div class="col-sm-6">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group">
									...
									<div class="col-xs-8">
										<input type="text" id="purchaseRate" name="purchaseRate" class="form-control" onkeydown="moveToNext(event, 'quantity')" placeholder="Pur. Rate" ref="purchaseRate" v-model="selectedProduct.Product_Purchase_Rate" v-on:input="productTotal" required />
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-3 no-padding-right">
										<input type="number" step="0.01" id="quantity" min="0" name="quantity" class="form-control" onkeydown="moveToNext(event, 'boxQty')" placeholder="Quantity" v-model="selectedProduct.pcs" v-on:input="productTotal" required />
									</div>
									<div class="col-sm-2">
	...
	new Vue({
		...
		methods: {
			...
			async removeFromCart(ind) {
				if (this.cart[ind].id && this.purchase.purchaseId != 0) {
					let stock = await axios.post('/get_product_stock', {
						productId: this.cart[ind].productId
					}).then(res => res.data);
					if (parseFloat(this.cart[ind].quantity) > parseFloat(stock)) {
